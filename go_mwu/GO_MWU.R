...
setwd('./go_mwu/')
input_files = c('meth.diff.csv',
                'mbd.log2FoldChange.csv',
                'mr.log2FoldChange.csv')
go_divisions = c('BP', 'MF', 'CC')
smallest_settings = c(25, 5, 5)

go_divisions = c('BP')
smallest_settings = c(50)
...
for (input in input_files){
  ...
  for (i in 1:length(go_divisions)){
    goDivision = go_divisions[i]
    smallest = smallest_settings[i]
    ...
    # Plotting results
    res_file = paste('MWU', goDivision, input, sep='_')
    res = read.table(res_file, header = TRUE)
  }
}

#selecct input/division combination and plot
#note mbd.log2FoldChange.csv and mr.log2FoldChange.csv had not significant terms
input = 'meth.diff.csv' 
goDivision = 'BP'
goDivision = 'MF'
goDivision = 'CC'
